        ...
        Configuration.getConfigurationByCode(TodayFormat(), function (err, end_cfg) {
         console.log(start_cfg+"...")
            ...
            if (req_type == "all_no_common") {
                console.log("-----")
                query = {common: false, type: req.query.type, create_at: {
                    "$gt": new Date(start_cfg.value),
                    "$lt": new Date(end_cfg.value)
                }};
            ...
            if (req_type == "common") {
                query = {common: true,type: req.query.type, create_at: {
                    "$gt": new Date(start_cfg.value),
                    "$lt": new Date(end_cfg.value)
                }};
            }
            ...
            if (req_type == "all_no_common_update") {
                query = {common: false, display:true, update:true, type: req.query.type, create_at: {
                    "$gt": new Date(start_cfg.value),
                    "$lt": new Date(end_cfg.value)
                }};
                templete = "back/postEx/content_update"
            ...
            if (req_type == "common_update") {
                query = {common: true, update: true,display:true, type: req.query.type, create_at: {
                    "$gt": new Date(start_cfg.value),
                    "$lt": new Date(end_cfg.value)
                }};
                templete = "back/postEx/content_update"
            ...
            if (req.query.update) {
                query = {update: req.query.update, common: req.query.common,diplay:true, type: req.query.type, create_at: {
                    "$gt": new Date(start_cfg.value),
                    "$lt": new Date(end_cfg.value)
                }};
                templete = "back/postEx/content_update"
            ...
            if (en_name) {
                query = {common: false, type: req.query.type,from_school_en_name:en_name, create_at: {
                    "$gt": new Date(start_cfg.value),
                    "$lt": new Date(end_cfg.value)
                }};
            }
                    ...
                    Configuration.getConfigurationByCode("end_time", function (err, end_cfg) {
                        ...
                        Configuration.getConfigurationByCode(TodayFormat(),function(err,configuration){
                            if(configuration){
                                ...
                                configuration.save(); return res.json({ success: true});
                            }else{
...
exports.result_post = function (req, res, next) {
    ...
    Configuration.getConfigurationByCode(DateFormat(day.value), function (err, start_cfg) {
        Configuration.getConfigurationByCode(TodayFormat(), function (err, end_cfg) {
            PostEx.getPostExsByQuery({'from_school_en_name': school_en_name, 'display': true, type: type, create_at: {
                "$gt": new Date(start_cfg.value),
                "$lt": new Date(end_cfg.value)
...
//                            result +="<p></p>"
                ...
                if (postexs.length < 22&&type!="info"&&type!="friend") {
                    PostEx.getPostExsByQuery({from_school_en_name: {$ne: school_en_name}, 'display': true, common: true, type: type, create_at: {
                        "$gt": new Date(start_cfg.value),
                        "$lt": new Date(end_cfg.value)
                    }}, options, function (err, postexs) {
