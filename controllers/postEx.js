...
exports.flex = function (req, res, next) {
    res.render('back/zipai/flex');
};
...
//请求参数
            ...
            utils.sendHttpRequest(apiurl, "POST", post_data, function (responseData) {
                ...
                if (code == 200) {
                    ...
                    if (action == 1) {
                        console.log("正常内容，通过")
                        return res.json({success: true,text:"正常内容，通过"});
                    } else if (action == 2) {

                        console.log("垃圾内容，删除")
                        return res.json({success: true,text:"垃圾内容，删除"});
                    } else if (action == 3) {
                        console.log("嫌疑内容")
                        return res.json({success: true,text:"嫌疑内容"});
                    }
                } else {
                    console.log('ERROR:code=' + code + ',msg=' + msg);
                }
                return res.json({success: true,text:"出现异常"});
            });
