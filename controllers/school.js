...
exports.showSearch = function (req, res, next) {
    var en_name=req.query.en_name;
    res.render('front/postEx/search',{en_name:en_name});

};


exports.Createsearch = function (req, res, next) {
    var cn_name = req.body.cn_name.trim();
    var en_name = req.body.en_name;
    console.log(cn_name)
    SchoolEx.getSchoolExsByQueryAndField({cn_name: cn_name}, '_id wx_account_name wx_account_id en_name cn_name erweima', {}, function (err, schools) {
        if (schools.length == 0) {
            res.render('front/postEx/searchResult', {school: null,en_name:en_name});
        }
        else {
            var school = schools[0]
            if (!fs.existsSync('./public/' + school.en_name + '.jpg')) {
                picUrl = './public/' + school.en_name + '.jpg';
                var fsStream = fs.createWriteStream(picUrl);
                var wxPicUrl = school.erweima
                var options = {
                    url: wxPicUrl,
                };
                ...
                stream.on('finish', function () {
                    fs.readFile(picUrl, function (err, data) {
                        ...
                        res.render('front/postEx/searchResult', {school: school,en_name:en_name});
                    });
                })
            } else {
                res.render('front/postEx/searchResult', {school: school,en_name:en_name});
        ...
        SchoolEx.getSchoolByEname(en_name, function (err, schoolEx) {
            if (school && schoolEx) {
                ...
                Post.getPostsByQuery({
                    'from_user.school_id': school._id,
                    create_at: {$gt: school.last_edit_at},
                    'display': true
                }, options, function (err, posts) {
                    var secret_index = 1;
                    var confess_index = 1;
