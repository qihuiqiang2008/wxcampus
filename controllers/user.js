...
exports.index = function (req, res, next) {
    ...
    if (type != "") {
        query = {"type": type, "from_user._id": req.session.user._id};
    }
    var page = parseInt(req.query.page, 10) || 1;
    page = page > 0 ? page : 1;
    var limit = 20;
    var options = { skip: (page - 1) * limit, limit: limit, sort: [ [ 'create_at', 'desc' ] ] };
    var proxy = EventProxy.create('user', 'posts', 'pages',
        function (user, posts, pages) {
            if (type == "") {
                res.render('front/user/index', {
                    user: user,
                    posts: posts,
                    pages: pages,
                    current_page: page,
    ...
    User.getUserById(req.session.user._id, proxy.done('user'));
    /*   PostLike.getCountByQuery({to_user: req.session.user._id, to_user_read: false}, proxy.done("liked_not_read_count"));
     PostReply.getCountByQuery({ $or: [
     { post_user_id: req.session.user._id, main_id: null, to_user_read: false} ,
     {to_user_id: req.session.user._id, to_user_read: false}
     ]}, proxy.done("replied_not_read_count"));*/
};
